...
export const MakkkhaPlaces = () => {
  const [delId, setdelId] = useState(null);
  const [showModal, setshowModal] = useState(false);
  const storeAllmakkkhaPlaces = useSelector(selectmakkkhaplaces);
  const [loading, setLoading] = useState(false);
  const navigate=useNavigate()
  console.log(storeAllmakkkhaPlaces)
  return (
    <>
      <div>
        <div >
          <div className="flex items-center justify-start gap-2  w-full">
            {storeAllmakkkhaPlaces && storeAllmakkkhaPlaces.length > 0 && <span className="font-bold"> {storeAllmakkkhaPlaces.length}</span>}
            <span className="font-bold text-2xl text-center w-full  text-purple-700">All MakkkhaPlaces</span>
          </div>
          {/* <h2 className="mr-3 font-bold text-2xl text-center mb-4 text-purple-700">All makkkhaPlaces</h2> */}
          {storeAllmakkkhaPlaces && storeAllmakkkhaPlaces.length > 0 &&
            <div className="text-right w-full flex items-center justify-end">
              <div className="flex items-center justify-center gap-2">
                ...
                <div className="cursor-pointer" onClick={() => {
                  saveMiqatsToJSON(storeAllmakkkhaPlaces, "makkkhaPlaces")
                }}>
        ...
        {storeAllmakkkhaPlaces && storeAllmakkkhaPlaces.length > 0 ?
          ...
          storeAllmakkkhaPlaces.map((makkkhaPlace, index) => {
            return <div key={index} className="px-8  py-4 shadow-lg border border-t-2 my-2 rounded-md grid grid-cols-5 md:grid-cols-6 text-center">
              <div className="text-sm px-2 font-medium col-span-1  text-gray-900">
                <img src={makkkhaPlace.icon} alt="icon" />
              </div>
              ...
              <div className="text-sm px-2 font-medium col-span-1  hidden md:block  text-gray-900">
                {makkkhaPlace.geometry.location.lat}
              </div>
              <div className="text-sm  flex items-center justify-center gap-2 px-2 font-medium col-span-1   text-gray-900">
                ...
                <span>
                  ({makkkhaPlace.user_ratings_total})
                </span>
              </div>
              ...
              <div className="text-sm font-medium col-span-1 flex items-center justify-center gap-2 text-gray-900">
                ...
                <button
                  onClick={() => {
                    navigate(`/Admin/editInfo/Makkah/${makkkhaPlace._id}`)
                  }}
              ...
              <div className="col-span-1  text-center flex justify-center ">
                <button onClick={() => {
                  openGoogleMaps(makkkhaPlace.geometry.location.lat, makkkhaPlace.geometry.location.lng)
              ...
              <div className="text-sm px-2 font-medium col-span-1   text-gray-900">
                {makkkhaPlace.makkkhaPlaceNameArb}
              </div>
              <div className="text-sm px-2 font-medium col-span-1    text-gray-900">

              </div>
              <div className="text-sm px-2 font-medium col-span-1 hidden md:block   text-gray-900">
                {makkkhaPlace.geometry.location.lng}
              </div>
