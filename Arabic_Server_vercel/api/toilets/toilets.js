...
const toilet = require("../../models/toiletsModel")
...
router.post('/delete_file_cloudinary', async (req, res) => {
    try {
        ...
        const existingtoilet = await toilet.findOne({ _id: toiletId });
        if (!existingtoilet) {
...
router.put('/:toiletId/edit-toilet', async (req, res) => {
    try {
        ...
        const existingtoilet = await toilet.findById(toiletId);
        if (!existingtoilet) {
            return res.status(404).json({ message: 'toilet not found' });
        }
...
router.get("/get-all-toilets", async (req, res) => {
    try {
        const toilets = await toilet.find().sort({ _id: -1 });
...
router.delete('/:toiletId/delete-toilet', async (req, res) => {
    const { toiletId } = req.params;
    try {
        const currenttoilet = await toilet.findOne({ _id: toiletId });
        ...
        await toilet.deleteOne({ _id: toiletId });
